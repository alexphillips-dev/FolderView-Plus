...
test('shared fatal banner runtime is exposed on settings and runtime pages and exposes fatal reporting helpers', () => {
    ...
    assert.match(folderPage, /FolderViewPlusFatalRuntimeContext[\s\S]*page:\s*'Folder Editor'[\s\S]*folderviewplus\.fatal-banner\.js/);
    assert.match(dashboardPage, /FolderViewPlusFatalRuntimeContext[\s\S]*page:\s*'Dashboard'[\s\S]*folderviewplus\.fatal-banner\.js/);
