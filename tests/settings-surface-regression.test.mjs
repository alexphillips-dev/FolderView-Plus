...
test('settings diagnostics exports client perf and theme telemetry helpers', () => {
    ...
    assert.doesNotMatch(supportBundlePreviewJs, /buildDiagnosticDomainsHtml|fv-support-bundle-domains/);
...
test('diagnostics tab uses a dedicated responsive workspace and support flow', () => {
    ...
    assert.match(settingsCss, /\.fv-diagnostics-core-progress\s*\{[\s\S]*width:\s*50%;[\s\S]*height:\s*8px;[\s\S]*margin:\s*0\.42rem auto 0;/);
    ...
    assert.match(diagnosticsJs, /checkedAtLabel:\s*formatCheckedAtLabel\(lastThemeDiagnostics\.generatedAt\)/);
    ...
    assert.match(settingsCss, /\.fv-support-bundle-overview\s*\{[\s\S]*grid-template-rows:\s*auto auto auto minmax\(0,\s*1fr\);/);
    assert.match(settingsCss, /\.fv-support-bundle-section-card\s*\{[\s\S]*grid-template-columns:\s*auto minmax\(0,\s*1fr\);[\s\S]*grid-template-rows:\s*auto minmax\(2\.1rem,\s*1fr\);[\s\S]*align-items:\s*start;[\s\S]*align-content:\s*stretch;[\s\S]*height:\s*100%;/);
    assert.match(settingsCss, /\.fv-support-bundle-section-badge\s*\{[\s\S]*grid-column:\s*1 \/ -1;[\s\S]*grid-row:\s*2;[\s\S]*align-self:\s*end;[\s\S]*justify-self:\s*center;/);
    ...
    assert.match(settingsCss, /\.fv-support-bundle-privacy-summary\s*\{[\s\S]*justify-content:\s*flex-start;[\s\S]*justify-self:\s*start;/);
    assert.match(settingsCss, /\.fv-support-bundle-privacy-items\s*\{[\s\S]*justify-content:\s*center;[\s\S]*justify-self:\s*center;/);
    assert.match(settingsCss, /\.fv-support-bundle-privacy-item strong\s*\{[\s\S]*border-radius:\s*6px;/);
    assert.match(settingsCss, /#fv-settings-root \.fv-support-bundle-redaction-card \.fv-support-bundle-privacy-disclosure\s*\{[\s\S]*color:\s*var\(--fvplus-settings-accent-strong\) !important;/);
