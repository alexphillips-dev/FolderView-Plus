...
DIAGNOSTICS_JS="${PLUGIN_DIR}/scripts/folderviewplus.activity-diagnostics.js"
...
require_contains "${SETTINGS_CSS}" '--fvplus-theme-text-primary:[[:space:]]*var\(--text,[[:space:]]*var\(--fvplus-settings-safe-text-primary\)\)' 'settings semantic primary token'
require_absent "${SETTINGS_CSS}" '--fvplus-theme-text-primary:[[:space:]]*var\(--text,[[:space:]]*currentColor\)' 'settings semantic primary token must use a theme-safe fallback'
...
require_contains "${SETTINGS_CSS}" '\.fv-setup-welcome-screen' 'wizard welcome screen selector'
require_contains "${SETTINGS_CSS}" '\.fv-setup-welcome-hero' 'wizard welcome hero selector'
require_absent "${SETTINGS_CSS}" '#fv-setup-assistant-dialog\[data-fv-wizard-contrast-tier=' 'wizard contrast tier selectors removed'
require_absent "${SETTINGS_CSS}" '\.fv-setup-contrast-field' 'wizard contrast field selector removed'
...
require_absent "${DIAGNOSTICS_JS}" 'run_theme_self_heal:[[:space:]]*Object\.freeze\(\{' 'retired theme self-heal recommendation mapping'
require_contains "${WIZARD_JS}" 'data-fv-focus-mode="\$\{focusModeEnabled \? '\''1'\'' : '\''0'\''\}"' 'wizard focus mode enabled shell state'
require_absent "${WIZARD_JS}" 'const normalizeSetupAssistantContrastPreference = \(value\) =>' 'wizard contrast preference normalizer removed'
require_absent "${WIZARD_JS}" 'const applySetupAssistantContrastTier = \(\) =>' 'wizard contrast apply helper removed'
require_absent "${WIZARD_JS}" 'id="fv-setup-focus-mode"' 'wizard focus mode control removed'
require_absent "${WIZARD_JS}" 'id="fv-setup-contrast-mode"' 'wizard contrast mode control removed'
require_absent "${SETTINGS_PAGE}" 'runThemeSelfHeal\(\)' 'old theme self-heal button wiring should be removed from settings page'
