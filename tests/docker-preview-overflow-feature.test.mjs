...
test('docker runtime applies preview row layout limits and keeps compact preview cards clickable', () => {
    ...
    assert.match(dockerJs, /const compactStatusMarkup = shouldHidePreviewStatus[\s\S]*\? ''[\s\S]*: `<span class="fv-preview-status-compact \$\{previewStateMeta\.className\}"/);
    ...
    assert.match(dockerPreviewActionsScript, /trigger\.dispatchEvent\(new MouseEventConstructor\('click'/);
    ...
    assert.match(dockerChildFolderPreviewMenuJs, /const resolveInputMethod = \(event = null\) => \{/);
    assert.match(dockerChildFolderPreviewMenuJs, /const resolveActivationPoint = \(event = null, \$item = null\) => \{/);
    assert.match(dockerChildFolderPreviewMenuJs, /const show = \(options = \{\}\) => \{/);
    assert.match(dockerChildFolderPreviewMenuJs, /addAction\('Expand to folder', 'fa-level-down'/);
    assert.match(dockerChildFolderPreviewMenuJs, /addAction\('Edit folder', 'fa-pencil'/);
    assert.match(dockerChildFolderPreviewMenuJs, /addAction\('Open folder actions', 'fa-bars'/);
    assert.match(dockerChildFolderPreviewMenuJs, /addAction\('Open folder actions', 'fa-bars', \(\) => \{[\s\S]*expandFolderPathToChild\(rootId, childId\);[\s\S]*scrollFolderRowIntoView\(childId\);[\s\S]*openFolderActions\(childId\);/);
    ...
    assert.match(dockerRuntimeHierarchyJs, /aria-haspopup="menu"/);
    assert.match(dockerRuntimeHierarchyJs, /childFolderPreviewMenuApi\?\.show\?\.\(\{/);
    assert.match(dockerRuntimeHierarchyJs, /click\.fvChildFolderPreviewMenu keydown\.fvChildFolderPreviewMenu/);
    assert.match(dockerRuntimeHierarchyJs, /contextmenu\.fvChildFolderPreviewMenu/);
    assert.match(dockerRuntimeHierarchyJs, /recordChildFolderPreviewRender\(\);/);
    assert.match(dockerRuntimeHierarchyJs, /recordChildFolderPreviewBinding\(\);/);
    ...
    assert.match(dockerJs, /recordChildFolderPreviewMenuOpen: \(details\) => getDockerPreviewActionsApi\(\)\?\.recordChildFolderPreviewMenuOpen\?\.\(details\)/);
    assert.match(dockerPreviewActionsScript, /childFolderPreview:\s*\{/);
    assert.match(dockerPreviewActionsScript, /recordChildFolderPreviewMenuOpen\(details = \{\}\) \{/);
