...
test('docker runtime actions refresh visible state in place instead of forcing a full reload', () => {
    ...
    assert.match(dockerRuntimeActionsJs, /const queueDockerDialogPostRenderRefresh = \(\) => \{[\s\S]*refreshDockerRuntimeState\(\{\s*followupDelayMs: DOCKER_DIALOG_RUNTIME_REFRESH_FOLLOWUP_DELAY_MS,\s*liveUpdateStatus: true\s*\}\)/);
