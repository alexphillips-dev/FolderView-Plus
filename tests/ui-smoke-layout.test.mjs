...
test('folder editor uses searchable parent picker and grouped tab panels', () => {
    ...
    assert.match(folderCss, /\.fv-section-shell\[data-section-shell="status"\] \.fv-editor-panel\[data-editor-panel="accent"\] \.fv-modern-field-row > dl > dd\.fv-accent-color-dd\s*\{[\s\S]*display:\s*inline-flex !important;[\s\S]*align-items:\s*center;[\s\S]*gap:\s*1\.2rem;/);
