...
(function fvplusDockerRuntimeSharedScope(window) {
    ...
    const getRuntimeCommandCenterCardTone = (summary) => {
    ...
    const getRuntimeCommandCenterStateLabel = (summary, options = {}) => {
        const total = Number(summary?.total || 0);
        const emptyLabel = String(options.emptyLabel || 'No items assigned');
        if (total <= 0) {
            return emptyLabel;
        }
        const runningLabel = String(options.runningLabel || 'running');
        const pausedLabel = String(options.pausedLabel || 'paused');
        const stoppedLabel = String(options.stoppedLabel || 'stopped');
        if (Number(summary?.running || 0) > 0) {
            return `${summary.running}/${total} ${runningLabel}`;
        }
        if (Number(summary?.paused || 0) > 0) {
            return `${summary.paused}/${total} ${pausedLabel}`;
        }
        return `${summary.stopped}/${total} ${stoppedLabel}`;
    };

    const buildRuntimeCommandCenterCardHtml = (options = {}) => {
        const cardClasses = [
            'fv-runtime-command-card',
            options.cardTone ? `is-${String(options.cardTone).trim()}` : '',
            options.pinned === true ? 'is-pinned' : '',
            options.locked === true ? 'is-locked' : '',
            options.focused === true ? 'is-focused' : '',
            options.focusHidden === true ? 'is-focus-hidden' : ''
        ].filter(Boolean).join(' ');
        const utilityPills = [
            options.pinned === true ? '<span class="fv-runtime-command-pill is-utility">Pinned</span>' : '',
            options.locked === true ? '<span class="fv-runtime-command-pill is-utility">Locked</span>' : '',
            options.focused === true ? '<span class="fv-runtime-command-pill is-utility">Focused</span>' : '',
            options.parentPillHtml ? String(options.parentPillHtml) : ''
        ].filter(Boolean).join('');
        const statsHtml = Array.isArray(options.stats)
            ? options.stats.map((entry = {}) => `<span class="fv-runtime-command-stat"><strong>${String(entry.value ?? '')}</strong><span>${String(entry.label ?? '')}</span></span>`).join('')
            : '';
        const folderId = String(options.folderId || '');
        const actionAttribute = String(options.actionAttribute || '').trim();
        const renderActionButtons = (actions = []) => actions.map((entry = {}) => {
            const label = String(entry.label || '');
            const action = String(entry.action || '').trim();
            const disabled = entry.disabled === true ? ' disabled' : '';
            return `<button type="button" ${actionAttribute}="${action}" data-folder-id="${folderId}"${disabled}>${label}</button>`;
        }).join('');
        return `<article class="${cardClasses}" data-folder-id="${folderId}">
        <div class="fv-runtime-command-card-head">
            <div class="fv-runtime-command-avatar">
                <img src="${String(options.iconSrc || '')}" alt="">
            </div>
            <div class="fv-runtime-command-headline">
                <div class="fv-runtime-command-title-row">
                    <h3>${String(options.titleHtml || '')}</h3>
                    <span class="fv-runtime-command-status is-${String(options.cardTone || 'empty')}">${String(options.statusHtml || '')}</span>
                </div>
                <div class="fv-runtime-command-subtitle">${String(options.subtitleHtml || '')}</div>
            </div>
        </div>
        ${utilityPills ? `<div class="fv-runtime-command-pill-row">${utilityPills}</div>` : ''}
        <div class="fv-runtime-command-stat-row">${statsHtml}</div>
        <div class="fv-runtime-command-actions">${renderActionButtons(options.primaryActions)}</div>
        <div class="fv-runtime-command-actions is-secondary">${renderActionButtons(options.secondaryActions)}</div>
        <div class="fv-runtime-command-members">${String(options.memberHtml || '')}</div>
    </article>`;
    };

    const createRuntimeCommandCenterController = (options = {}) => {
        const hostId = String(options.hostId || '').trim();
        const runtimeType = String(options.runtimeType || '').trim();
        const actionAttribute = String(options.actionAttribute || '').trim();
        const resolveTable = typeof options.resolveTable === 'function'
            ? options.resolveTable
            : (() => null);
        const resolveAddButton = typeof options.resolveAddButton === 'function'
            ? options.resolveAddButton
            : (() => null);
        ...
        const setNativeVisibility = (hidden) => {
            ...
            const addButton = resolveAddButton();
        ...
        const bindActions = (host) => {
            if (!(host instanceof HTMLElement) || !actionAttribute || host.dataset.fvCommandCenterBound === 'true') {
                return;
            }
            host.dataset.fvCommandCenterBound = 'true';
            host.addEventListener('click', async (event) => {
                const actionTarget = event.target instanceof Element
                    ? event.target.closest(`[${actionAttribute}]`)
                    : null;
                if (!(actionTarget instanceof HTMLElement) || !host.contains(actionTarget)) {
                    return;
                }
                event.preventDefault();
                try {
                    await onAction({
                        action: String(actionTarget.getAttribute(actionAttribute) || '').trim().toLowerCase(),
                        folderId: String(actionTarget.getAttribute('data-folder-id') || '').trim(),
                        target: actionTarget,
                        host,
                        event
                    });
                } catch (error) {
                    console.error(`folderview.plus ${runtimeType || 'runtime'} command center action failed`, error);
                }
            });
        };

        const render = (renderOptions = {}) => {
            const enabled = renderOptions.enabled === true;
            const host = enabled ? ensureHost() : document.getElementById(hostId);
            const folderCount = Number(renderOptions.folderCount || 0);
            const shouldRender = enabled && host instanceof HTMLElement && folderCount > 0;
            setNativeVisibility(shouldRender);
            ...
            bindActions(host);
            const kicker = String(renderOptions.kicker || '').trim();
            const summaryHtml = String(renderOptions.summaryHtml || '');
            const toolbarHtml = String(renderOptions.toolbarHtml || '');
            const cardsHtml = String(renderOptions.cardsHtml || '');
            host.hidden = false;
            host.innerHTML = `<div class="fv-runtime-command-center-header">
        <div>
            ${kicker ? `<div class="fv-runtime-command-center-kicker">${kicker}</div>` : ''}
            ${summaryHtml ? `<div class="fv-runtime-command-center-summary">${summaryHtml}</div>` : ''}
        </div>
        ${toolbarHtml ? `<div class="fv-runtime-command-center-toolbar">${toolbarHtml}</div>` : ''}
    </div>
    ...
    window.FolderViewDockerRuntimeShared = {
        ...
        getRuntimeCommandCenterCardTone,
        getRuntimeCommandCenterStateLabel,
        buildRuntimeCommandCenterCardHtml,
        createRuntimeCommandCenterController,
