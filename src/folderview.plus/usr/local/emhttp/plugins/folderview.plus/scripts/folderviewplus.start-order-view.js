...
(function(root, factory) {
    ...
    const createApi = (deps = {}) => {
        ...
        const buildHeaderSummaryHtml = (infoByName = {}) => {
            ...
            return `<div class="fv-start-order-summary-card" aria-label="${text('settings.start-order.summary', 'Docker container summary')}">
                <div class="fv-start-order-summary-metric"><i class="fa fa-cubes" aria-hidden="true"></i><span><strong>${rows.length}</strong><small>${text('settings.start-order.total-containers', 'Total containers')}</small></span></div>
                <div class="fv-start-order-summary-divider" aria-hidden="true"></div>
                <div class="fv-start-order-summary-metric"><i class="fa fa-play" aria-hidden="true"></i><span><strong>${autostart}</strong><small>${text('settings.start-order.autostart-containers', 'Autostart containers')}</small></span></div>
            </div>`;
        };
            ...
            return `<div class="fv-docker-start-order-top" data-fv-start-order-region="top">
                <div class="fv-docker-start-order-config">
                    <div class="fv-docker-start-order-controls">
                        <label class="setting-select"><span>${text('settings.start-order.mode', 'Start order mode')}</span><span class="fv-start-order-select-shell"><i class="fa fa-list" aria-hidden="true"></i><select id="docker-start-order-mode" data-fv-onchange="updateDockerStartOrderMode(this.value)">
                            <option value="unmanaged" ${plan.mode === 'unmanaged' ? 'selected' : ''}>${text('settings.start-order.unmanaged', 'Leave Unraid order unmanaged')}</option>
                            <option value="docker-page" ${plan.mode === 'docker-page' ? 'selected' : ''}>${text('settings.start-order.docker-page', 'Follow Docker page order')}</option>
                            <option value="custom-batches" ${plan.mode === 'custom-batches' ? 'selected' : ''}>${text('settings.start-order.custom', 'Custom batch order')}</option>
                        </select><i class="fa fa-chevron-down" aria-hidden="true"></i></span></label>
                        <label class="setting-select"><span>${text('settings.start-order.remaining', 'Remaining autostart containers')}</span><span class="fv-start-order-select-shell"><i class="fa fa-users" aria-hidden="true"></i><select id="docker-start-order-remaining" data-fv-onchange="updateDockerStartOrderRemaining(this.value)">
                            <option value="after" ${plan.remaining === 'after' ? 'selected' : ''}>${text('settings.start-order.after', 'Start after custom batches')}</option>
                            <option value="before" ${plan.remaining === 'before' ? 'selected' : ''}>${text('settings.start-order.before', 'Start before custom batches')}</option>
                            <option value="keep" ${plan.remaining === 'keep' ? 'selected' : ''}>${text('settings.start-order.keep', 'Keep their current relative order')}</option>
                        </select><i class="fa fa-chevron-down" aria-hidden="true"></i></span></label>
                    </div>
                    <div class="fv-docker-start-order-toolbar">
                        <button type="button" class="fv-docker-start-order-primary" data-fv-onclick="refreshDockerStartOrderPreview()"><i class="fa fa-eye" aria-hidden="true"></i> ${text('settings.start-order.preview', 'Preview order')}</button>
                        <button type="button" data-fv-onclick="syncDockerStartOrderNow()" ${plan.mode === 'unmanaged' ? `disabled title="${text('settings.start-order.unmanaged-title', 'Unraid owns this order in unmanaged mode')}"` : ''}><i class="fa fa-refresh" aria-hidden="true"></i> ${text('settings.start-order.sync', 'Sync now')}</button>
                    </div>
                </div>
        ...
        const buildBatchHtml = (batch, index, options) => {
            ...
            }).join('') : `<div class="fv-docker-start-order-empty-copy">${text('settings.start-order.add-items', 'Add folders or containers to this batch.')}</div>`;
        ...
        const buildBatchesHtml = (batches, options) => `<section class="fv-docker-start-order-batch-section" data-fv-start-order-region="batches"><div class="fv-start-order-section-head"><strong><i class="fa fa-cubes" aria-hidden="true"></i> ${text('settings.start-order.custom-batches', 'Custom startup batches')}</strong><button type="button" data-fv-onclick="addDockerStartOrderBatch()"><i class="fa fa-plus" aria-hidden="true"></i> ${text('settings.start-order.add-batch', 'Add batch')}</button></div><div class="fv-docker-start-order-batches">${batches.length ? batches.map((batch, index) => buildBatchHtml(batch, index, options)).join('') : `<div class="fv-docker-start-order-empty"><span class="fv-docker-start-order-empty-icon"><i class="fa fa-cube" aria-hidden="true"></i></span><span><strong>${text('settings.start-order.no-batches', 'No custom batches yet')}</strong><small>${text('settings.start-order.no-batches-help', 'Add a batch to define exact boot groups.')}</small></span></div>`}</div></section>`;

        const buildPreviewPlaceholderHtml = () => `<section id="docker-start-order-preview" class="fv-docker-start-order-preview" data-fv-start-order-region="preview"><div class="fv-recovery-empty-state"><strong>${text('settings.start-order.preview-pending', 'Preview has not loaded yet.')}</strong><span>${text('settings.start-order.preview-help', 'Use Preview order to inspect the exact autostart sequence.')}</span></div></section>`;
        ...
        const buildSequenceHtml = (preview, infoByName) => {
            ...
            if (!sequence.length) return `<div class="fv-recovery-empty-state"><strong>${text('settings.start-order.none', 'No autostart containers detected.')}</strong><span>${text('settings.start-order.enable-help', 'Enable Docker autostart for a container to include it in the sequence.')}</span></div>`;
            return `<div class="fv-start-order-table-wrap"><div class="fv-start-order-table"><div class="fv-start-order-table-head"><span>Order</span><span>Container</span><span>Wait time</span><span>Enabled</span><span aria-hidden="true"></span></div><ol class="fv-docker-start-order-sequence">${sequence.map((entry, index) => {
