...
test('privacy mode persists in prefs normalization on server and client', () => {
    ...
    assert.match(utilsNormalizationJs, /privacyMode:\s*false/);
    assert.match(utilsNormalizationJs, /privacyMaskNames:\s*true/);
    assert.match(utilsNormalizationJs, /privacyMaskContainerIps:\s*true/);
    assert.match(utilsNormalizationJs, /privacyMaskLocalIps:\s*true/);
    assert.match(utilsNormalizationJs, /privacyMaskPorts:\s*true/);
    assert.match(utilsNormalizationJs, /const PRIVACY_MODE_PREFS_SCHEMA = 3/);
    assert.match(utilsPrefsJs, /const privacyModePrefsReady = runtimePrefsSchema >= PRIVACY_MODE_PREFS_SCHEMA/);
    assert.match(utilsPrefsJs, /privacyMode:\s*![\s\S]*hasOwnProperty\.call\(incomingDashboard, 'privacyMode'\)[\s\S]*privacyModePrefsReady && incomingDashboard\.privacyMode === true/);
    ...
    assert.match(utilsPrefsJs, /privacyMaskNames:\s*![\s\S]*hasOwnProperty\.call\(incomingDashboard, 'privacyMaskNames'\)[\s\S]*incomingDashboard\.privacyMaskNames !== false/);
    assert.match(utilsPrefsJs, /privacyMaskContainerIps:\s*![\s\S]*hasOwnProperty\.call\(incomingDashboard, 'privacyMaskContainerIps'\)[\s\S]*incomingDashboard\.privacyMaskContainerIps !== false/);
    assert.match(utilsPrefsJs, /privacyMaskLocalIps:\s*![\s\S]*hasOwnProperty\.call\(incomingDashboard, 'privacyMaskLocalIps'\)[\s\S]*incomingDashboard\.privacyMaskLocalIps !== false/);
    assert.match(utilsPrefsJs, /privacyMaskPorts:\s*![\s\S]*hasOwnProperty\.call\(incomingDashboard, 'privacyMaskPorts'\)[\s\S]*incomingDashboard\.privacyMaskPorts !== false/);
});
...
test('docker runtime privacy toggle stays in sync with saved dashboard privacy prefs', () => {
    ...
    assert.match(runtimeSharedControlsJs, /const createStableToggleController = \(options = \{\}\) =>/);
    assert.match(runtimeSharedControlsJs, /if \(!input \|\| !shell\.contains\(input\)\) \{[\s\S]*shell\.innerHTML = String\(buildMarkup\(state\) \|\| ''\);/);
    assert.match(runtimeSharedControlsJs, /const boundShells = new WeakSet\(\);/);
    assert.match(runtimeSharedControlsJs, /const boundPrimaryInputs = new WeakSet\(\);/);
    assert.match(runtimeSharedControlsJs, /\.off\('change\.fvplusStableToggle'\)[\s\S]*\.on\('change\.fvplusStableToggle', handleChange\);/);
    assert.match(runtimeSharedControlsJs, /if \(!synchronizingPrimary\) \{[\s\S]*invoke\(onToggle, input\.checked === true\);/);
    assert.match(runtimeSharedControlsJs, /input\.dispatchEvent\(new EventConstructor\('change', \{ bubbles: true \}\)\);/);
    assert.match(runtimeSharedControlsJs, /initializeSwitch\(input, state\);\s*bindPrimaryEvents\(input\);/);
