...
const FOLDER_AUTO_RULE_REGEX_KINDS = type === 'docker'
    ? Object.freeze(['name_regex', 'image_regex', 'compose_project_regex'])
    : Object.freeze(['name_regex']);
const FOLDER_AUTO_RULE_KIND_LABELS = Object.freeze({
...
let folderEditorPrefs = utils && typeof utils.normalizePrefs === 'function'
...
let folderEditorRuleDraft = {
    effect: 'include',
    kind: FOLDER_AUTO_RULE_REGEX_KINDS[0] || 'name_regex',
    pattern: ''
};
...
const normalizeFolderEditorRuleKind = (kind) => {
    ...
    return Object.prototype.hasOwnProperty.call(FOLDER_AUTO_RULE_KIND_LABELS, normalized)
        ? normalized
        : (FOLDER_AUTO_RULE_REGEX_KINDS[0] || 'name_regex');
};

const getFolderEditorRuleKindLabel = (kind) => FOLDER_AUTO_RULE_KIND_LABELS[normalizeFolderEditorRuleKind(kind)] || FOLDER_AUTO_RULE_KIND_LABELS.name_regex;
...
const getFolderEditorRuleIssues = (rule) => {
    ...
    const kind = normalizeFolderEditorRuleKind(rule?.kind);
    const pattern = String(rule?.pattern || '').trim();
    if (FOLDER_AUTO_RULE_REGEX_KINDS.includes(kind)) {
        if (!pattern) {
...
const getFolderEditorAutoRules = () => {
    ...
    if (!activeFolderEditorFolderId) {
        return [];
    }
    return rules.filter((rule) => String(rule?.folderId || '').trim() === activeFolderEditorFolderId);
};
...
const loadFolderEditorPrefs = async ({
    ...
    folderEditorPrefsLoadPromise = (async () => {
        ...
        folderEditorPrefs = utils.normalizePrefs(response?.prefs || {});
...
const saveFolderEditorPrefs = async (nextPrefs) => {
    ...
    folderEditorPrefs = utils.normalizePrefs(response?.prefs || nextPrefs);
    folderEditorPrefsLoaded = true;
    return folderEditorPrefs;
};
...
const renderFolderAutoRulesPanel = () => {
    if (!modernFolderEditorEnabled) {
        return;
    }
    const body = document.querySelector('.fv-section-shell[data-section-shell="rules"] .fv-section-shell-body');
    if (!(body instanceof HTMLElement)) {
        return;
    }
    let panel = document.getElementById('fvFolderAutoRulesPanel');
    if (!(panel instanceof HTMLElement) || panel.parentElement !== body) {
    ...
    const kinds = FOLDER_AUTO_RULE_REGEX_KINDS.map((kind) => `
    ...
    } else if (!activeFolderEditorFolderId) {
        bodyHtml = buildFolderAutoRulesEmptyStateHtml(
    ...
    if (panel.dataset.bound !== '1') {
        $(panel)
            ...
            .on('change', '#fvFolderAutoRulesKind', function onFolderRuleKindChange() {
                ...
                const pattern = document.getElementById('fvFolderAutoRulesPattern');
...
const refreshFolderAutoRulesPanel = async ({
    forceReload = false
} = {}) => {
    renderFolderAutoRulesPanel();
    if (!modernFolderEditorEnabled || !activeFolderEditorFolderId) {
        return;
    }
    if (!requestClient || typeof requestClient.getJson !== 'function') {
        return;
    ...
    try {
        ...
        if (folderEditorRulesMessage.tone === 'error' && folderEditorRulesMessage.text.includes('Failed to load')) {
            setFolderEditorRulesMessage('');
        }
    } catch (error) {
...
const addFolderEditorAutoRule = async () => {
    if (!activeFolderEditorFolderId || folderEditorRulesBusy) {
        return;
    }
    const patternInput = document.getElementById('fvFolderAutoRulesPattern');
    ...
    try {
        const nextRule = {
            ...
            enabled: true,
            folderId: activeFolderEditorFolderId,
