...
const runRuntimeThemeChecks = async (page, { label, browserName, mobile, zoom, type }) => {
    const collectReport = async () => page.evaluate((runtimeType) => {
        ...
        const button = Array.from(document.querySelectorAll('button.folder-dropdown')).find((node) => isVisible(node));
        if (!button) {
            return { skipped: true, reason: `No visible ${runtimeType} folder dropdown.` };
        }
        const errors = [];
        ...
        if (!fg) {
            errors.push('Could not parse dropdown color.');
        } else {
            const ratio = contrast({ ...fg, a: 1 }, bg);
            if (ratio < 2.4) {
                errors.push(`Dropdown contrast too low (${ratio.toFixed(2)}).`);
            }
        }
        const rect = button.getBoundingClientRect();
        if (rect.width < 10 || rect.height < 10) {
            errors.push(`Dropdown hit target too small (${Math.round(rect.width)}x${Math.round(rect.height)}).`);
        }
        return { skipped: false, errors };
    }, type);
    ...
    if (type !== 'dashboard') {
        const dropdown = page.locator('button.folder-dropdown').filter({ has: page.locator('i.fa') }).first();
...
const runFolderEditorThemeChecks = async (page, { label, browserName, mobile, zoom, type }) => {
    const report = await page.evaluate((context) => {
        const parseColor = (raw) => {
            const value = String(raw || '').trim().toLowerCase();
            if (!value || value === 'transparent') {
