    ...
    function getDiagnosticsSnapshot(string $privacyMode = FVPLUS_DIAGNOSTICS_DEFAULT_PRIVACY): array {
        ...
        foreach ($types as $type) {
            ...
            $typesData[$type] = [
                ...
                'pinnedFolderCount' => count($prefs['pinnedFolderIds'] ?? []), 'pinnedFolderIds' => normalizeStringIdList($prefs['pinnedFolderIds'] ?? []),
                'hiddenFolderCount' => count($prefs['hiddenFolderIds'] ?? []), 'hiddenFolderIds' => normalizeStringIdList($prefs['hiddenFolderIds'] ?? []),
