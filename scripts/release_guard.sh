...
SOURCE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS="${ROOT_DIR}/src/folderview.plus/usr/local/emhttp/plugins/folderview.plus/scripts/docker.runtime.child-folder-preview-menu.js"
...
if [[ ! -f "${SOURCE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS}" ]]; then
  echo "ERROR: Missing source child-folder preview menu helper script: ${SOURCE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS}" >&2
  exit 1
fi
...
REQUIRED_ARCHIVE_PATHS=(
  ...
  "./usr/local/emhttp/plugins/folderview.plus/scripts/docker.runtime.child-folder-preview-menu.js"
...
TMP_ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS="$(mktemp)"
...
if ! grep -Fxq "${ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS_PATH}" <<< "${ARCHIVE_LIST}"; then
  ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS_PATH="${ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS_PATH#./}"
fi
...
tar -xOf "${ARCHIVE_FILE}" "${ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS_PATH}" > "${TMP_ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS}"
...
if ! env_truthy "${FVPLUS_ALLOW_PACKAGED_SOURCE_DRIFT:-0}"; then
  ...
  if ! text_files_match "${SOURCE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS}" "${TMP_ARCHIVE_DOCKER_CHILD_FOLDER_PREVIEW_MENU_JS}"; then
    fail_packaged_source_mismatch "Packaged docker.runtime.child-folder-preview-menu.js does not match source docker.runtime.child-folder-preview-menu.js."
  fi
