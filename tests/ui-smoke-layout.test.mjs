...
test('settings page includes smoke-test-critical containers and scripts', () => {
    ...
    assert.doesNotMatch(settingsPage, /id="vm-health-critical-threshold"/);
    assert.doesNotMatch(settingsPage, /id="vm-health-profile"/);
    assert.doesNotMatch(settingsPage, /id="vm-health-updates-mode"/);
    assert.doesNotMatch(settingsPage, /id="vm-health-all-stopped-mode"/);
...
test('nested folder expansion avoids duplicate parent previews and keeps child-only reveal path', () => {
    ...
    assert.match(dockerPreviewActionsJs, /const ensureDockerPreviewActionSlot = \(\$target,\s*actionName,\s*factory\) =>/);
    assert.match(dockerPreviewActionsJs, /const reconcileDockerPreviewActionButtons = \(\$target,[\s\S]*options = \{\}\) =>/);
    ...
    assert.match(dockerJs, /const appendDockerPreviewActionButtons = \(\$target,\s*settings = \{\},\s*containerName = '',\s*shellValue = '\/bin\/sh',\s*webuiUrl = '',\s*options = \{\}\) =>/);
    ...
    assert.match(dockerJs, /appendDockerPreviewActionButtons\([\s\S]*\$targetForAppend,[\s\S]*folder\.settings,[\s\S]*ct\.info\.Name,[\s\S]*ct\.info\.Shell,[\s\S]*previewWebuiUrl,[\s\S]*webuiCapability/);
...
test('folder editor uses the modern stylesheet without retired runtime alignment overrides', () => {
    ...
    assert.match(folderPage, /class="fv-action-dialog-layout"/);
    assert.match(folderPage, /class="fv-action-type-options" role="radiogroup"/);
    assert.match(folderPage, /data-action-type="0" role="radio" aria-checked="true"/);
    assert.match(folderPage, /data-action-type="1" role="radio" aria-checked="false"/);
    assert.match(folderPage, /data-action-target-command="all"/);
    assert.match(folderPage, /data-action-target-command="clear"/);
    assert.match(folderPage, /class="fv-action-icon-preview"/);
    assert.doesNotMatch(folderPage, /name="action_type"[^>]*onchange=/);
    assert.doesNotMatch(folderPage, /name="action_standard"[^>]*onchange=/);
    assert.match(folderCss, /\.fv-action-dialog-field-grid\s*\{[\s\S]*?grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\);/);
    ...
    assert.match(folderJs, /const syncActionType = \(nextType = modeSelect\.val\(\)\) =>/);
    assert.match(folderJs, /dialog\.find\('\[constraint\*="action-type-"\]'\)\.hide\(\);/);
    assert.match(folderJs, /\.attr\('tabindex', selected \? '0' : '-1'\);/);
    assert.match(folderJs, /const validateActionDialog = \(showErrors = validationTouched\) =>/);
    ...
    assert.match(folderCss, /\.item\.fv-member-row-draggable\s*\{[\s\S]*cursor:\s*default;/);
    ...
    assert.match(folderCss, /\.fv-modern-field-row\.is-name-row input\[type="text"\]\s*\{[\s\S]*?width:\s*min\(100%, 420px\) !important;[\s\S]*?max-width:\s*min\(100%, 420px\) !important;[\s\S]*?font-size:\s*1\.3rem;/);
...
test('folder editor exposes folder-scoped advanced auto-rules for saved folders', () => {
    ...
    assert.match(folderRulesJs, /Create managed include or exclude rules without relying on the legacy folder regex\./);
    assert.match(folderRulesJs, /<dt>Auto-rules:<\/dt>/);
    ...
    assert.match(folderCss, /\.fv-section-shell\[data-section-shell="rules"\] \.fv-section-shell-body\.fv-section-panel-grid\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\)/);
...
test('folder editor uses searchable parent picker and grouped tab panels', () => {
    ...
    assert.match(folderChromeJs, /rules:\s*\[[\s\S]*key:\s*'auto-rules'[\s\S]*keepEmpty:\s*true[\s\S]*key:\s*'regex'/);
...
test('settings runtime uses extracted chrome module and shared request wrapper', () => {
    ...
    assert.match(settingsJs, /class="status-cell"><span class="status-cell-content \$\{statusDisplayClass\}">\$\{statusSummaryChipHtml\}\$\{statusBreakdownHtml\}\$\{statusTrendHtml\}/);
