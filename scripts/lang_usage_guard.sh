...
for (const fullPath of sourceFiles.sort()) {
  ...
  const applicationWrapperRegex = /\b(?:setupAssistantT|importT|folderEditorT|dashboardT|dockerT|diagnosticsT|fatalBannerT|startOrderT|translateVmText|translateSettingsText|translate)\(\s*['"]([^'"]+)['"]/g;
