...
test('decomposition preserves every historical public PHP function name', () => {
    ...
    for (const [facade, baseline] of Object.entries(expected)) {
        ...
        const additions = new Set(['fvplusFolderView3ResolveOrder', 'fvplusFolderView3GroupStyles', 'fvplus_localize_response_keys', 'fvplus_attach_response_translations', 'parseBatchManualOrder', 'assertBatchFolderOperationCount', 'assertBatchManualOrderRevision', 'applyBatchManualOrderPrefs']);
