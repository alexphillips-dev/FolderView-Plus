...
test('docker runtime applies preview row layout limits and keeps compact preview cards clickable', () => {
    ...
    assert.match(dockerRuntimeHierarchyJs, /const getFolderPreviewDescendants = \(folderId\) => \{/);
    ...
    assert.match(dockerRuntimeHierarchyJs, /for \(const descendant of getFolderPreviewDescendants\(id\)\)/);
