...
test('import modal is compact, bounded, and responsive', () => {
    ...
    assert.match(css, /#import-preview-dialog \*:not\(i\):not\(\.import-mode-choice-badge\),[\s\S]*?font-size:\s*1\.2rem !important;/);
    assert.match(css, /#import-preview-dialog \.import-summary-total strong\s*\{[\s\S]*?font-size:\s*1\.55rem !important;/);
    assert.match(css, /#import-preview-dialog \.import-summary-breakdown > span,[\s\S]*?#import-preview-dialog \.import-disclosure > summary small,[\s\S]*?font-size:\s*1\.2rem !important;/);
    assert.match(css, /#import-preview-dialog \.import-mode-choice-badge\s*\{[\s\S]*?font-size:\s*1rem !important;/);
    ...
    assert.match(runtime, /element\.classList\?\.contains\('import-mode-choice-badge'\)/);
});
