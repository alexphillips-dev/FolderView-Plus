# FolderView Plus Docs

This folder holds project documentation that does not need to stay at the repository root.

## Core Docs

- [USER_GUIDE.md](USER_GUIDE.md)
- [INSTALLATION_AND_UPGRADES.md](INSTALLATION_AND_UPGRADES.md)
- [MIGRATION_GUIDE.md](MIGRATION_GUIDE.md)
- [PRIVACY.md](PRIVACY.md)
- [COMPATIBILITY.md](COMPATIBILITY.md)
- [CHANGELOG.md](CHANGELOG.md)
- [SUPPORT_POLICY.md](SUPPORT_POLICY.md)
- [DEPRECATIONS.md](DEPRECATIONS.md)
- [THEME_GUIDE.md](THEME_GUIDE.md)
- [THEME_API_CONTRACT.md](THEME_API_CONTRACT.md)
- [TRANSLATING.md](TRANSLATING.md)
- [TRANSLATION_PLATFORM.md](TRANSLATION_PLATFORM.md)
- [TROUBLESHOOTING.md](TROUBLESHOOTING.md)

## Architecture

- [ARCHITECTURE.md](ARCHITECTURE.md)
- [api-endpoint-contract.md](api-endpoint-contract.md)
- [docker-runtime-architecture.md](docker-runtime-architecture.md)
- [durable-storage.md](durable-storage.md)
- [runtime-host-adapter.md](runtime-host-adapter.md)
- [shared-ui-primitives.md](shared-ui-primitives.md)
- [vm-runtime-architecture.md](vm-runtime-architecture.md)
- [visual-runtime-contract.md](visual-runtime-contract.md)

## Testing and Assets

- [`current-state.json`](current-state.json) — machine-readable names and behavior contracts used to prevent documentation drift.
- [edge-cases-test-matrix.md](edge-cases-test-matrix.md)
- [runtime-performance-budgets.md](runtime-performance-budgets.md)
- [images/README.md](images/README.md)
- [images/screenshots/README.md](images/screenshots/README.md)

## Project and Support

- [Contributing](../.github/CONTRIBUTING.md)
- [Security policy](../.github/SECURITY.md)
- [Support channels](../.github/SUPPORT.md)
- [Versioned icon asset packs](../asset-packs/README.md)

## Releases

- Versioned release notes live in [`releases/`](releases/).
